 ...
 *
 */
class EventEmbeddedActiveMQChannel implements EventChannel, MessageListener {

  private static final int WORKER_THREAD_COUNT = 20;
  private static final int EVENT_QUEUE_CAPACITY = 20000;
  static EventEmbeddedActiveMQChannel INSTANCE = new EventEmbeddedActiveMQChannel();
  ...
  private EventEmbeddedActiveMQChannel() {
    try {
      ...
      ActiveMQConnectionFactory factory = new ActiveMQConnectionFactory(ub.toString());
      conn = factory.createConnection();
      session = conn.createSession(false, Session.AUTO_ACKNOWLEDGE);
      conn.start();

      producer = session.createProducer(session.createTopic("event-stream"));
      producer.setDeliveryMode(DeliveryMode.PERSISTENT);

      MessageConsumer consumer = session.createConsumer(session.createTopic("event-stream"));
      consumer.setMessageListener(this);

      initializeThreadPool(
          configuration.get("events.channel.default.worker.pool.size", WORKER_THREAD_COUNT + ""));
    } catch (Exception e) {
