...
class EventActiveMQChannel implements EventChannel, MessageListener {
  ...
  private EventActiveMQChannel() {
    try {
      ...
      Properties mqChannelInfo = EventEngineConfig.INSTANCE.getMQChannelConfig();
      ActiveMQConnectionFactory factory = new ActiveMQConnectionFactory(
          mqChannelInfo.getProperty("event.channel.mq.address", "tcp://127.0.0.1:61616"));
      conn = factory.createConnection();
      session = conn.createSession(false, Session.AUTO_ACKNOWLEDGE);
      conn.start();

      producer = session.createProducer(
          session.createTopic(mqChannelInfo.getProperty("event.channel.mq.topic", "event-stream")));
      ...
      initializeThreadPool(
          mqChannelInfo.getProperty("event.channel.worker.pool.size", WORKER_THREAD_COUNT + ""));
    } catch (Exception e) {
